...
class SelectNISTdata(Form):
    ...
    * extension methods:
    ...
    code_b = StringField('', validators.Length( min=4, max=5, message=_(u'Check input') ) )
    element_b = StringField('', validators.Length( min=1, max=2, message=_(u'Enter Al, W, Ni, Fe or Cu') ))
    ####
    ...
    code = ListWidget(vasp,Dmol3,PWscf)
    ##
    ...
    Fe = StringField('Fe')
    Cu = StringField('Cu')
    elements = ListWidget(Al, W, Ni, Fe, Cu)
    ##
    ####
    ...
    properties = ListWidget(B, dB, E0, a0)
